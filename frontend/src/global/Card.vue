<template>
    <div
        ...
        style="width: 250px; height: calc(100% - 1rem);"
    >
        <img :src="getImageUrl(this.img)" class="card-img-top card-image-big" />
        <div class="card-body">
    ...
    <div
        v-else
        class="card m-2 hover-pointer hover-shadow"
        style="width: 175px; height: calc(100% - 1rem);"
    >
        <img
            :src="getImageUrl(this.img)"
            class="card-img-top card-image-small"
        />
        <div class="card-body">
            <h5 class="card-title">{{ this.title }}</h5>
            <h6 class="card-title">At the {{ this.location }}</h6>
...
01/22/20 - Jian Chan - Finalize file, methods, and fix layout

File Creation Date: 01/20/20
Development Group: GetUP
Client Group: UP Diliman Students
Purpose: Reusable card component to display previews of workouts
*/
const Card = {
    name: "Card",
    props: {
